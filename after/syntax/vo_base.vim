"#########################################################################
"# File: after/syntax/vo_base.vim
"# Description: Improved VimOutliner syntax highlighting
"# Version: 0.3.4
"#
...
"# CHANGELOG {{{1
"#
"#   V0.1.0 Pre-alpha Steve Litt, 5/28/2001
"#     - Set of outliner friendly settings
"#
"#   V0.1.1 Pre-alpha Steve Litt, 5/28/2001
"#     - No change
"#
"#   V0.1.2 Pre-alpha Steve Litt, 5/30/2001
"#     - No Change
"#
"#   V0.1.3 Pre-alpha Steve Litt, 5/30/2001
"#     - No Change
"#
"#   V0.2.0           Noel Henson, 11/24/2002
"#     - Added code for outliner-friendly expand and
"#       collapse, comma comma commands, color coding, hooks for a
"#       spellchecker, sorting, and date insertion.
"#
"#   V0.2.1      Bill Powell, 1/23/2007 21:23 http://www.billpowellisalive.com
...
"#       a VO file.
"#     - Different schemes will need tweaking.
"#     - Occasionally certain groups will be rendered invisible. ;)
"#
"#   V0.3.0      Bill Powell, 4/30/2007 9:36 http://www.billpowellisalive.com
"#     - Changed a few linked groups to reduce chances of groups being
"#       invisible.
"#     - No longer use Ignore group for anything.
"#     - Still a little redundancy; different groups might linked to same color
"#       group. E.g., PT1 and UT1. But some color schemes (e.g. astronout) will
"#       differentiate between Special and Debug. Others will use the same
"#       colors for, say, Identifier and Debug. It just depends.
...
"#
"#   V0.3.4           Horacio Sanson, 1/19/2010 00:41
"#     - Clean up this file.
"#     - Allow different syntax highlithing of Preformatted User Defined Blocks
"#       depending on the contained code.
"#     - This allows to have blocks with C, CPP, Ruby etc. code and let vim use
"#       their corresponding syntax files to highlight their contents.
"#     - Check the UserBlockSnip for details on how to add support for other
"#       languages.
...
"Colors linked {{{1
	...
	"color for user text (non-wrapping) snippets
	hi UBS guifg=white ctermfg=white
...
" Headings {{{2
syntax region OL1 start=+^[^:\t]+ end=+^[^:\t]+me=e-1 contains=outlTags,BT1,BT2,PT1,PT2,TA1,TA2,UT1,UT2,UB1,UB2,UBS,spellErr,SpellErrors,BadWord,OL2 keepend
syntax region OL2 start=+^\t[^:\t]+ end=+^\t[^:\t]+me=e-2 contains=outlTags,BT2,BT3,PT2,PT3,TA2,TA3,UT2,UT3,UB2,UB3,UBS,spellErr,SpellErrors,BadWord,OL3 keepend
...
" Function to generate different syntax highlighting within User Defined Blocks.
" Based on http://vim.wikia.com/wiki/VimTip857
function! UserBlockSnip(filetype) abort
  let ft=toupper(a:filetype)
  let group='textGroup'.ft

  " Remove current syntax definition because some syntax files do nothing if
  " it is defined.
  ...
  " Load the main systax file
  execute 'syntax include @'.group.' syntax/'.a:filetype.'.vim'

  " Load any custom syntax file if available
  try
    execute 'syntax include @'.group.' after/syntax/'.a:filetype.'.vim'
  catch
  endtry

  " Restore the previous syntax definition
  if exists('s:current_syntax')
   ...
   execute 'syntax region UBS start=/^\t*<'.ft.'$/ end=/^\(\t*<.\{}\)\@!/ contains=@'.group.' contained'
endfunction

" Generate the syntax regions for the laguages we want to use.
" TODO: How to get a list of all possible syntax files and call this method on
" them all??
call UserBlockSnip('c')
call UserBlockSnip('sh')
call UserBlockSnip('cpp')
call UserBlockSnip('ruby')
call UserBlockSnip('python')
call UserBlockSnip('perl')
call UserBlockSnip('ragel')
